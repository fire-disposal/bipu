# 网络层架构文档

## 概述

新的网络层架构围绕自动生成的 `RestClient` 构建，提供统一的异常处理、Token 管理、日志输出和简单方便的请求接口。

## 架构图

```
┌─────────────────────────────────────────────────────────────┐
│                    Features (页面/组件)                      │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│              Services (业务服务层)                           │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ AuthService  │  │ProfileService│  │  ImService   │ ...  │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│         Generated API Clients (自动生成的 API 客户端)        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │Authentication│  │UserProfile   │  │  Messages    │ ...  │
│  │   Client     │  │   Client     │  │   Client     │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│                    ApiClient                                │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ • Dio 初始化                                         │  │
│  │ • 拦截器管理                                        │  │
│  │ • 异常转换                                          │  │
│  │ • 请求执行                                          │  │
│  └──────────────────────────────────────────────────────┘  │
└────────────────────────┬────────────────────────────────────┘
                         │
        ┌────────────────┼────────────────┐
        │                │                │
┌───────▼────────┐ ┌────▼──────────┐ ┌──▼──────────────┐
│ ApiInterceptor │ │ TokenManager  │ │ ApiException    │
│                │ │               │ │                 │
│ • Token 管理   │ │ • 保存 Token  │ │ • 异常体系      │
│ • 错误处理     │ │ • 获取 Token  │ │ • 错误转换      │
│ • 日志输出     │ │ • 清除 Token  │ │ • 错误消息      │
│ • 401 处理     │ │ • Token 过期  │ │                 │
└────────────────┘ └───────────────┘ └─────────────────┘
        │                │                │
        └────────────────┼────────────────┘
                         │
        ┌────────────────┼────────────────┐
        │                │                │
┌───────▼────────┐ ┌────▼──────────┐ ┌──▼──────────────┐
│  StorageManager│ │  FlutterSecure│ │  Logger         │
│                │ │  Storage      │ │                 │
│ • 安全存储     │ │               │ │ • 彩色日志      │
│ • Token 存储   │ │ • 加密存储    │ │ • 详细信息      │
│ • 缓存管理     │ │ • 平台适配    │ │ • 错误追踪      │
└────────────────┘ └───────────────┘ └─────────────────┘
```

## 核心组件

### 1. [`api_exception.dart`](api_exception.dart) - 异常处理

定义了统一的异常体系：

- **`ApiException`** - 基类
- **`NetworkException`** - 网络异常（超时、连接错误等）
- **`AuthException`** - 认证异常（401、403、Token 过期等）
- **`ServerException`** - 服务器异常（5xx 错误）
- **`ValidationException`** - 验证异常（400 错误）
- **`ParseException`** - 解析异常

### 2. [`api_interceptor.dart`](api_interceptor.dart) - 拦截器

处理以下功能：

- **Token 管理**：自动附加 Bearer Token 到请求头
- **公开端点白名单**：跳过不需要认证的接口
- **错误处理**：捕获 401 错误并清除本地认证信息
- **日志输出**：使用 `logger` 包输出详细的请求/响应日志

### 3. [`api_client.dart`](api_client.dart) - API 客户端

围绕生成的 `RestClient` 的封装：

- **单例模式**：全局唯一实例
- **Dio 初始化**：配置基础 URL、超时时间、拦截器
- **异常转换**：将 `DioException` 转换为 `ApiException`
- **请求执行**：提供 `execute()` 方法统一处理请求

### 4. [`token_manager.dart`](token_manager.dart) - Token 管理器

统一处理 Token 的获取、保存、刷新和清除：

- **保存 Token**：`saveTokens(accessToken, refreshToken)`
- **获取 Token**：`getAccessToken()` / `getRefreshToken()`
- **清除 Token**：`clearTokens()`
- **检查 Token**：`hasToken()`
- **Token 过期**：`handleTokenExpired()` + `tokenExpired` 通知

## 使用指南

### 基础使用

#### 1. 登录

```dart
import 'package:flutter_user/core/network/network.dart';

try {
  final token = await ApiClient.instance.api.authentication.postApiPublicLogin(
    body: UserLogin(username: 'user@example.com', password: 'password123'),
  );
  
  // 保存 Token
  await TokenManager.saveTokens(
    accessToken: token.accessToken,
    refreshToken: token.refreshToken,
  );
  
  print('登录成功');
} on AuthException catch (e) {
  print('认证失败: ${e.message}');
} on NetworkException catch (e) {
  print('网络错误: ${e.message}');
}
```

#### 2. 获取当前用户信息

```dart
try {
  final user = await ApiClient.instance.api.userProfile.getApiProfileMe();
  print('用户名: ${user.username}');
} on AuthException catch (e) {
  // Token 过期或无效，需要重新登录
  print('需要重新登录: ${e.message}');
}
```

#### 3. 发送消息

```dart
try {
  final message = await ApiClient.instance.api.messages.postMessage(
    body: MessageCreate(content: 'Hello, World!', recipientId: 'user_id_123'),
  );
  print('消息已发送: ${message.id}');
} on ValidationException catch (e) {
  print('验证失败: ${e.message}');
  if (e.errors != null) {
    print('错误详情: ${e.errors}');
  }
}
```

### 高级使用

#### 1. 自定义请求执行

```dart
final apiClient = ApiClient.instance;

try {
  final result = await apiClient.execute(
    () => apiClient.api.users.searchUsers(q: 'john'),
    operationName: 'SearchUsers',
  );
} on ApiException catch (e) {
  print('API 错误: ${e.message}');
}
```

#### 2. 异常处理

```dart
try {
  await ApiClient.instance.api.authentication.postApiPublicLogin(
    body: UserLogin(username: 'user', password: 'pass'),
  );
} catch (e) {
  if (e is AuthException) {
    print('🔐 认证异常: ${e.message}');
  } else if (e is ServerException) {
    print('🖥️ 服务器异常: ${e.message}');
  } else if (e is NetworkException) {
    print('🌐 网络异常: ${e.message}');
  } else if (e is ValidationException) {
    print('✓ 验证异常: ${e.message}');
  }
}
```

#### 3. Token 管理

```dart
// 检查 Token 是否存在
final hasToken = await TokenManager.hasToken();

// 获取 Token
final token = await TokenManager.getAccessToken();

// 清除 Token
await TokenManager.clearTokens();

// 监听 Token 过期事件
TokenManager.tokenExpired.addListener(() {
  if (TokenManager.tokenExpired.value) {
    // 重定向到登录页
    print('Token 已过期，请重新登录');
  }
});
```

#### 4. 添加自定义拦截器

```dart
final apiClient = ApiClient.instance;

// 添加自定义拦截器
apiClient.addInterceptor(MyCustomInterceptor());

// 移除拦截器
apiClient.removeInterceptor(myInterceptor);

// 清除所有拦截器
apiClient.clearInterceptors();

// 重置 Dio 实例
apiClient.reset();
```

## 特性

### ✅ Token 管理

- 自动从 `StorageManager` 读取 Token
- 自动附加到请求头
- 支持公开端点白名单
- 401 错误时自动清除本地 Token
- Token 过期通知机制

### ✅ 错误处理

- 统一的异常体系
- 详细的错误信息
- 自动异常转换
- 支持自定义错误处理
- 错误堆栈跟踪

### ✅ 日志输出

- 使用 `logger` 包输出彩色日志
- 请求/响应详细信息
- 错误堆栈跟踪
- 仅在 Debug 模式下输出

### ✅ 简单方便

- 单例模式，全局访问
- 直接访问生成的 API 客户端
- 自动异常处理
- 支持自定义拦截器

## 配置

### 基础 URL

基础 URL 通过环境变量配置：

```bash
flutter run --dart-define=API_BASE_URL=https://api.example.com
```

默认值：`https://api.205716.xyz`

### 超时时间

在 [`api_client.dart`](api_client.dart) 中修改：

```dart
connectTimeout: const Duration(seconds: 15),
receiveTimeout: const Duration(seconds: 15),
sendTimeout: const Duration(seconds: 15),
```

### 日志级别

在 [`api_interceptor.dart`](api_interceptor.dart) 中修改 `Logger` 配置。

## 最佳实践

1. **使用 TokenManager**：统一通过 `TokenManager` 管理 Token
2. **异常处理**：始终使用 try-catch 处理 API 调用
3. **Token 管理**：不要手动管理 Token，由拦截器自动处理
4. **日志输出**：使用 `logger` 包而不是 `print()`
5. **错误消息**：使用异常的 `message` 属性获取用户友好的错误消息
6. **业务服务**：在各自的 Service 中调用 API，不要在 UI 层直接调用

## 迁移指南

### 从旧网络层迁移

#### 旧代码
```dart
import 'package:flutter_user/core/network/network_service.dart';

final networkService = NetworkService.instance;
final user = await networkService.api.userProfile.getMe();
```

#### 新代码
```dart
import 'package:flutter_user/core/network/network.dart';

final user = await ApiClient.instance.api.userProfile.getApiProfileMe();
```

## 常见问题

### Q: 如何处理 Token 过期？

A: 当收到 `AuthException` 时，应该清除本地数据并重定向到登录页。拦截器会自动清除 Token，`TokenManager.tokenExpired` 会发出通知。

### Q: 如何添加自定义请求头？

A: 在 `ApiClient._initializeDio()` 中修改 `BaseOptions.headers`。

### Q: 如何处理文件上传？

A: 使用生成的 API 方法，例如 `api.userProfile.postApiProfileAvatar(file: file)`。

### Q: 如何调试网络请求？

A: 启用 Debug 模式，日志会自动输出到控制台。

### Q: Token 存储在哪里？

A: Token 存储在 `StorageManager` 的安全存储中，使用 `FlutterSecureStorage` 加密存储。

## 相关文件

- [`api_exception.dart`](api_exception.dart) - 异常定义
- [`api_interceptor.dart`](api_interceptor.dart) - 拦截器实现
- [`api_client.dart`](api_client.dart) - API 客户端
- [`token_manager.dart`](token_manager.dart) - Token 管理器
- [`network.dart`](network.dart) - 导出文件
- `package:flutter_user/core/api/export.dart` - 生成的 API 导出

## 支持

如有问题或建议，请联系开发团队。
