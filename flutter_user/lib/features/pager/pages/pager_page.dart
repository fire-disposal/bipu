...
class _PagerPageState extends State<PagerPage> {
  ...
  Widget _buildVoiceMode(BuildContext context) {
    ...
    final op =
        ...
        _config.getOperator(_config.defaultOperator)!;
    final Map<String, dynamic>? opCfg = _assistant.getCurrentOperatorConfig();

    return Column(
      children: [
        Card(
          color: op.themeColor.withOpacity(0.02),
              ...
              children: [
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child:
                        opCfg != null && (opCfg['portrait'] as String?) != null
                        ? Image.asset(
                            opCfg['portrait'] as String,
                            fit: BoxFit.cover,
                            width: 56,
                            height: 56,
                            errorBuilder: (c, e, s) =>
                                Text(op.name.isNotEmpty ? op.name[0] : 'O'),
                          )
                        : Text(
                          ...
                          IconButton(
                            ...
                            onPressed: () async {
                              await Navigator.of(context).push(
                                MaterialPageRoute(
                                  builder: (_) => const OperatorGallery(),
                                ),
                              );
                              setState(() {});
                            },
          ...
          child: Row(
            children: [
              ElevatedButton.icon(
                onPressed: () async {
                  try {
                    await _assistant.speakScript('greeting');
                  } catch (_) {}
                },
                icon: const Icon(Icons.play_arrow),
                label: const Text('播放接线员引导'),
                style: ElevatedButton.styleFrom(backgroundColor: op.themeColor),
              ),
              const SizedBox(width: 12),
              Text(
                'TTS Voice SID: ${_assistant.getCurrentOperatorConfig()?['ttsSid'] ?? 0}',
