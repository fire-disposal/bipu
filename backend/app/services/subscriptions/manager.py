...
class SubscriptionManager:
    ...
    def _register_handlers(self):
        """注册所有订阅处理器"""
        self.register_handler("天气推送", WeatherSubscriptionHandler())
        self.register_handler("今日运势", FortuneSubscriptionHandler())
        
    def register_handler(self, name: str, handler: BaseSubscriptionHandler):
    ...
    def process_subscriptions(self, subscriptions: List[UserSubscription], db: Session) -> Dict[str, int]:
        ...
        for sub in subscriptions:
            try:
                # 1. 获取对应的处理器
                handler_name = sub.subscription_type.name
                handler = self.handlers.get(handler_name)
                
                if not handler:
                    logger.warning(f"未找到订阅类型处理器: {handler_name}")
                    continue
                ...
                message = Message(
                    ...
                    pattern=pattern,
                    category=msg_data.get("category", "subscription"),
