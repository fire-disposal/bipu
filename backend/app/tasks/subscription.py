...
def generate_weather_subscription():
    """生成天气订阅消息"""
    return _process_subscription_task("天气推送")

@shared_task
def generate_fortune_subscription():
    """生成今日运势订阅消息"""
    return _process_subscription_task("今日运势")

def _process_subscription_task(subscription_name: str):
    """处理特定类型的订阅任务通用逻辑"""
    db = None
    try:
        db = SessionLocal()
        ...
        subscriptions = db.query(UserSubscription).join(
            ...
            SubscriptionType.name == subscription_name,
            UserSubscription.is_enabled == True
        ).all()
        
        if not subscriptions:
            logger.info(f"没有启用的 {subscription_name} 订阅")
        ...
        logger.info(f"{subscription_name} 订阅消息生成完成: 创建了 {result['created_count']} 条消息")
        return result
        
    except Exception as e:
        logger.error(f"生成 {subscription_name} 订阅消息失败: {e}")
        raise
    finally:
        if db:
            db.close()
